...
<?php
$schedule_4 = $this->ManageProgress_model->get_schedule(array("schedule.progress_id" => $progress_4->progress_id, "schedule.transaction_id" => $transaction->transaction_id))[0];
?>
...
<?php else: ?>
    ...
    <script>
        $(document).on('click', '#step_4_return', function () {
            $.ajax({
                "method": "POST",

                "url": '<?= base_url() ?>' + "/ManageProgress/step_4_return_exec",
                "dataType": "JSON",
                "data": {
                    'transaction_id': '<?= $transaction->transaction_id ?>',
                    'progress_id': '<?= $progress_3->progress_id ?>',
                    'schedule_id': '<?= $schedule_4->schedule_id ?>'
                },
                    ...
                    <?php else: ?>
                        <div class="card-footer small text-muted text-center">
                            <div class="btn-group" role="group" aria-label="Approval">
                                <button type ="button" class = "px-5 py-2 input-group-addon btn btn-outline-danger" data-toggle = "modal"  title = "Return to Step 3" data-target = "#step_4_sched_return"><i class = "fa fa-chevron-left"></i> Return to Step 3</button>     
                                <button type ="button" class = "px-5 py-2 input-group-addon btn btn-outline-secondary" data-toggle = "modal"  title = "Leave a remark" data-target = "#step_4_sched_disapprove"><i class = "fa fa-comment"></i> Leave a remark</button>     
                                <button type ="button" class = "px-5 py-2 input-group-addon btn btn-outline-primary" data-toggle = "modal"  title = "Proceed to Step 5" data-target = "#step_4_sched_approve"><i class = "fa fa-chevron-right"></i> Proceed to Step 5</button>
                            </div>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    <!-- MODAL FOR RETURNING FROM STEP 3 -->
    <div class="modal fade" id="step_4_sched_return" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form id = "step_2_form_d" method = "POST" role = "form">
            <!-- Displayed Fields -->
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="eventHeader">Return to Step 3</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        Are you sure you want to return to Step 3? All progress on this step will be lost.
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
                        <button type = "button" id ="step_4_return" class="btn btn-danger">Yes</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    <div class="modal fade multi-step" id="step_4_sched_approve" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-dialog modal-lg"role="document">
            <div class="modal-content" >
                <div class="modal-header">
                    <h5 class="modal-title step-1" data-step="1"> Set schedule for Visiting Chosen Adoptee #1</h5>
                    <h5 class="modal-title step-2" data-step="2"> Set schedule for Visiting Chosen Adoptee #2</h5>
                    <h5 class="modal-title step-3" data-step="3"> Set schedule for Visiting Chosen Adoptee #3</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body step step-1">
                    <p class="text-muted"><i class="fa fa-check"></i> Before approving Step 4 (Home Visit), set schedule for Visit Chosen Adoptee #1 for the next step.</p>
                ...
                <div class="modal-body step step-2">
                    <p class="text-muted"><i class="fa fa-check"></i> Before approving Step 4 (Home Visit), set schedule for Visit Chosen Adoptee #2 for the next step.</p>
                    <form role ="form" method="POST">
                        <div class = "form-row">
                            <div class = "col-md-6 form-group">
                ...
                <div class="modal-body step step-3">
                    <p class="text-muted"><i class="fa fa-check"></i> Before approving Step 4 (Home Visit), set schedule for Visit Chosen Adoptee #3 for the next step.</p>
                    <form role ="form" method="POST">
                        <div class = "form-row">
                            <div class = "col-md-6 form-group">
                        ...
                        <div class = "form-row">
                            <label for="comment_step_4">Remarks</label>
                            <textarea class = "form-control" id = "comment_step_4" name = "comment_step_4" placeholder = "Leave a remark here." required=""></textarea>
                        </div>
                    </form>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id = "step_4_approve" class="btn btn-primary step step-3" data-step="3" onclick="nextStep_step_4(3)">Set Schedule #3</button>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class = "form-row">
                            ...
                            <textarea class = "form-control" id = "comment_step_4_d" name = "comment_step_4_d" placeholder = "Leave a remark here." required=""></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="button" id = "step_4_disapprove" class="btn btn-danger">Submit</button>
                    </div>
                </div>
            </div>
        </form>
                    ...
                    case 1:
                    {
                        $.ajax({
                            ...
                            "data": {
                                ...
                                'schedule_desc': "Home Visit is completed! Visiting Chosen Adoptee will be the next step for <?= $transaction->user_firstname . " " . $transaction->user_lastname ?> to adopt <?= $transaction->pet_name ?>.",
                    ...
                    case 2:
                    {
                        $.ajax({
                            ...
                            "data": {
                                ...
                                'schedule_desc': "Visiting Chosen Adoptee will be the next step for <?= $transaction->user_firstname . " " . $transaction->user_lastname ?> to adopt <?= $transaction->pet_name ?>.",
                                'schedule_color': "#1e7e34",
