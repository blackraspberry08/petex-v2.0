...
//        echo "</pre>";
        ...
        $current_user = $this->ManageUsers_model->get_users("user", array("user_id" => $this->session->userdata("userid")))[0];
        $transactionId = $this->PetAdoption_model->fetch("transaction", array("user_id" => $this->session->userdata("userid"), "pet_id" => $selectedPetId))[0];
...
//[Docuement_Root] sa pag Up
    ...
    public function petAdoptionOnlineFormRepeat_exec() {
        $this->session->set_userdata("petadopterid", $this->uri->segment(3));
        redirect(base_url() . "PetAdoption/petAdoptionOnlineFormRepeat");
    }

    public function petAdoptionOnlineFormRepeat() {
        $errors = $this->session->userdata("validationErrors");
        $values = $this->session->userdata("validationValues");
        $selectedPetId = $this->session->userdata("petadopterid");
        $userInfo = $this->PetAdoption_model->getinfo('user', array('user_id' => $this->session->userid))[0];
        ...
        $this->load->view("pet_adoption/includes/header", $data);
        $this->load->view("user_nav/navheader");
        $this->load->view("pet_adoption/petAdoptionOnlineFormRepeat");
        $this->load->view("pet_adoption/includes/footer");
    }

    public function petAdoptionOnlineFormRepeat_send() {
        $animal_id = $this->uri->segment(3);
        $this->form_validation->set_rules('numhome', "Tel No. (Home)", "numeric");
        $this->form_validation->set_rules('numwork', "Tel No. (Work)", "numeric");
        $this->form_validation->set_rules('nummobile', "Mobile No.", "required|regex_match[^(09|\+639)\d{9}$^]");
...
//ERROR IN FORM
            ...
            $this->session->set_flashdata("validationErrors", $errors);
            $this->session->set_flashdata("validationValues", $values);
            $this->petAdoptionOnlineFormRepeat_exec();
        } else {
